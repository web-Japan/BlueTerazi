...
(() => {
  ...
  // 2枚目の演出背景は index.html と同階層の transition-bg.jpg
  document.documentElement.style.setProperty(
    "--transition-bg-url",
    'url("transition-bg.jpg?v=12")'
  );
  ...
  yesButton.addEventListener("click", () => {
    ...
    window.setTimeout(() => {
      try {
        sessionStorage.setItem("blueTempleEntryToken", "ok");
        sessionStorage.setItem("blueTempleEntryTime", String(Date.now()));
      } catch (e) {}
      window.location.href = "answer.html?v=12";
    }, 7200);
  });
})();
